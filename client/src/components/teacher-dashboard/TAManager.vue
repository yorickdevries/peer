                ...
                <b-card header="Teaching Assistants">
                    ...
                    >
                        <template slot="actions" slot-scope="data">
                            <b-button @click="removeTA(data.item.user_netid)" size="sm" variant="danger"
                                >Remove
                            </b-button>
                        </template>
                    </b-table>
                    ...
                    <!--Pagination-->
                    <b-pagination
                        :total-rows="this.teachingAssistants.length"
                        :per-page="Number(perPage)"
                        v-model="currentPage"
                        class="my-0"
                    />
...
export default {
    ...
    methods: {
        ...
        async fetchTeachingAssistants() {
            try {
                let { data } = await api.getEnrolledUsersWithRole(this.$route.params.courseId, "teachingassistant")
